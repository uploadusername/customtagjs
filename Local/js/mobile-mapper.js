...
class mapper extends HTMLTableElement {
    constructor() {
        mapper.info('mapper created');
    ...
    connectedCallback() {
        try {
            ...
            this.innerHTML = +'mapper connected';
            if (typeof this.renderContent === 'function')
                this.renderContent();
        } catch (err) {
            document.getElementById("mapper1").innerHTML = err.message + ' <br> ';
        }
    }

    renderContent() {
        this.innerHTML = +"mapper rendered";
...
customElements.define('mapper', mapper, {
    extends: 'table'
});