...
class template extends HTMLTableElement {
    constructor() {
        template.info('template created');
    ...
    connectedCallback() {
        try {
            ...
            this.innerHTML = +'template connected';
            if (typeof this.renderContent === 'function')
                this.renderContent();
        } catch (err) {
            document.getElementById("template1").innerHTML = err.message + ' <br> ';
        }
    }

    renderContent() {
        this.innerHTML = +"template rendered";
