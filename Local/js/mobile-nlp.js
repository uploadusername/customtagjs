...
class nlp extends HTMLTableElement {
    constructor() {
        nlp.info('nlp created');
    ...
    connectedCallback() {
        try {
            ...
            this.innerHTML = +'nlp connected';
            if (typeof this.renderContent === 'function')
                this.renderContent();
        } catch (err) {
            document.getElementById("nlp1").innerHTML = err.message + ' <br> ';
        }
    }

    renderContent() {
        this.innerHTML = +"nlp rendered";
