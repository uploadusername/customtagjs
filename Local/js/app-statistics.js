...
class statistics extends HTMLTableElement {
    constructor() {
        statistics.info('statistics created');
    ...
    connectedCallback() {
        try {
            ...
            this.innerHTML = +'statistics connected';
            if (typeof this.renderContent === 'function')
                this.renderContent();
        } catch (err) {
            document.getElementById("statistics1").innerHTML = err.message + ' <br> ';
        }
    }

    renderContent() {
        this.innerHTML = +"statistics rendered";
...
customElements.define('statistics', statistics, {
    extends: 'table'
});