...
class map extends HTMLTableElement {
    constructor() {
        map.info('map created');
    ...
    connectedCallback() {
        try {
            ...
            this.innerHTML = +'map connected';
            if (typeof this.renderContent === 'function')
                this.renderContent();
        } catch (err) {
            document.getElementById("map1").innerHTML = err.message + ' <br> ';
        }
    }

    renderContent() {
        this.innerHTML = +"map rendered";
