...
class workflow extends HTMLTableElement {
    constructor() {
        workflow.info('workflow created');
    ...
    connectedCallback() {
        try {
            ...
            this.innerHTML = +'workflow connected';
            if (typeof this.renderContent === 'function')
                this.renderContent();
        } catch (err) {
            document.getElementById("workflow1").innerHTML = err.message + ' <br> ';
        }
    }

    renderContent() {
        this.innerHTML = +"workflow rendered";
...
customElements.define('workflow', workflow, {
    extends: 'table'
});