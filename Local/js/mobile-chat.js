...
class chart extends HTMLTableElement {
    constructor() {
        chart.info('chart created');
    ...
    connectedCallback() {
        try {
            ...
            this.innerHTML = +'chart connected';
            if (typeof this.renderContent === 'function')
                this.renderContent();
        } catch (err) {
            document.getElementById("chart1").innerHTML = err.message + ' <br> ';
        }
    }

    renderContent() {
        this.innerHTML = +"chart rendered";
